...
StyleInstructions.prototype.renderRule = function(item,intend){
  ...
  if ( (typeof item.variable === 'boolean') && ( item.variable === false ) ){
    ...
    if (typeof unit==='string'){
      result.push( intend + 'style["'+item.name+'-unit"] = ' + unit + ';' );
    }
  }
...
StyleInstructions.prototype.getUnit = function(item){
  try{
    if (typeof item.value.value[0].value[0].unit === 'string'){
      return '"'+item.value.value[0].value[0].unit+'"';
    }
  }catch(e){
    //console.log(item.value);
    return unit;
